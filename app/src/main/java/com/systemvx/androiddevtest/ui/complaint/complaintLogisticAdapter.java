...
public class complaintLogisticAdapter extends BaseQuickAdapter<complaint_item, BaseViewHolder> {
    private Context context;
    private List<complaint_item> data;

    public complaintLogisticAdapter(Context context, int layoutResId, @Nullable List<complaint_item> data) {
    ...
    @Override
    protected void convert(@NonNull BaseViewHolder helper, @NonNull complaint_item item) {
        //获取物流信息和物流时间的字体颜色, 最新的一条物流数据字体为绿色
        int newInfoColor = context.getResources().getColor(helper.getLayoutPosition() == 0 ? R.color.colo_blue : R.color.white_shadow);
        ...
        helper.setGone(R.id.iv_new, helper.getLayoutPosition() == 0 && data.size() > 1)
                ...
                //物流信息
                .setText(R.id.tv_info, item.getAcceptState())
                //物流时间
